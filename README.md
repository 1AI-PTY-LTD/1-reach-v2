...
740 tests. Run with `-v` for verbose output or `--cov` for a coverage report. If the schema has changed since the last run, rebuild the test database first:
...
4. The Stripe API version is pinned in `StripeMeteredBillingProvider.STRIPE_API_VERSION` to prevent breaking changes from Stripe API updates. Update this version deliberately when upgrading, after verifying compatibility.
...
1. **Stripe Dashboard → Webhooks → Add endpoint:**
   ...
   - Subscribe to: `invoice.paid`, `invoice.payment_failed`, `invoice.voided`
