...
| Celery beat | — | Runs `dispatch_due_messages` every 60 s, `reconcile_stale_sent` (every 15 min — polls provider for missed callbacks on schedules stuck SENT >24 h), `cleanup_stale_media_blobs` (daily — deletes MMS media blobs for failed schedules >7 days old), `generate_monthly_invoices` (1st of month — creates Stripe invoices for subscribed orgs) |
