...
- When a send registers a callback with the provider, set `callback_registered=True` on the returned `SendResult` (and the top-level key of bulk-send dicts) — `reconcile_stale_sent` uses it to decide how soon to poll (`RECONCILE_NO_CALLBACK_AFTER_MINUTES` vs `RECONCILE_STALE_AFTER_HOURS`)
