...
| `FREE_CREDIT_AMOUNT` | No | Dollar credits granted to new orgs on signup (default: `5.00`) |
...
| `MESSAGE_QUEUED_TIMEOUT_MINUTES` | No | Minutes before a stuck QUEUED schedule is re-dispatched (default: `5`) |
| `MESSAGE_PROCESSING_TIMEOUT_MINUTES` | No | Minutes before a stuck PROCESSING schedule is recovered (default: `10`) |
| `FRONTEND_URL` | No | Frontend origin used for Stripe Checkout redirects (default: `http://localhost:5173`) |
| `CSRF_TRUSTED_ORIGINS` | No | Comma-separated origins trusted for CSRF — required for the Django admin in deployed environments |
...
| `WELCORP_TWO_WAY_JOB_TYPE` | No | Welcorp `job_type` literal for reply-capable sends (default: `2 Way SMS`) — env-overridable because Welcorp's docs are inconsistent about its casing; see the RUNBOOK "Two-way SMS" probe notes |
| `TWO_WAY_REPLY_WINDOW_HOURS` | No | Hours the provider keeps the pooled reply number routing back to us — Welcorp job field `expires` (default: `24`) |
| `INBOUND_OPT_OUT_KEYWORDS` | No | Comma-separated reply keywords that opt the sender out, matched exact after strip/upper (default: `STOP`). Handled locally per-org — Welcorp's account-level `optout_code` is deliberately never used |
| `BASE_URL` | No | Publicly accessible base URL for this application, **including the scheme** (e.g. `https://your-domain.com`) — used for provider delivery callbacks. Leave unset for local dev (localhost can't receive callbacks; each send logs a WARNING and the reconcile poll resolves delivery status instead) |
| `RECONCILE_STALE_AFTER_HOURS` | No | Hours a schedule may sit in SENT before `reconcile_stale_sent` polls the provider — applies to sends that registered a delivery callback (default: `2`) |
| `RECONCILE_NO_CALLBACK_AFTER_MINUTES` | No | Minutes before polling a send that registered **no** callback — polling is the only status path for those (default: `5`) |
...
| Celery beat | — | Runs `dispatch_due_messages` every 60 s, `reconcile_stale_sent` (every 15 min — polls the provider for schedules stuck in SENT past their stale window: `RECONCILE_NO_CALLBACK_AFTER_MINUTES` for sends with no callback registered, `RECONCILE_STALE_AFTER_HOURS` otherwise), `cleanup_stale_media_blobs` (daily — deletes MMS media blobs for failed schedules >7 days old), `generate_monthly_invoices` (1st of month — creates Stripe invoices for subscribed orgs) |
...
**Delivery status tracking:** The `SMSProvider` base class also defines a provider-agnostic delivery callback/polling interface. Providers can override `parse_delivery_callback()` to handle incoming webhooks, `validate_callback_request()` for authentication, `get_callback_url()` to register callbacks in send payloads, and `poll_job_status()` to fetch delivery reports on demand. All methods return `DeliveryEvent` objects consumed by the `process_delivery_event` Celery task, which updates schedule status and triggers billing refunds on carrier-reported failures. Each send records whether a delivery callback was registered (`SendResult.callback_registered` → `Schedule.callback_registered`). Note: Welcorp's `GET /jobs/{id}` does **not** echo the stored callback fields (always null — confirmed with Welcorp support on a job whose callback demonstrably fired), so registration cannot be verified via their API; the flag reflects what was attached at send time. The `reconcile_stale_sent` beat task polls the provider for schedules stuck in SENT past an adaptive stale window: sends with **no** callback registered (polling is their only status path — e.g. local dev, or a `BASE_URL`/`WELCORP_CALLBACK_SECRET` config regression) are polled after `RECONCILE_NO_CALLBACK_AFTER_MINUTES` (default 5), while callback-backed sends wait `RECONCILE_STALE_AFTER_HOURS` (default 2) since polling is just the safety net there. The Welcorp provider (`welcorp.py`) implements all four methods. Welcorp reports recipient numbers in international-without-plus format (`61XXXXXXXXX`) in both callbacks and job reports — `_normalise_phone()` converts these to the local `04XXXXXXXX` format used to match schedules. Welcorp's `SENT` status means "carrier accepted" (the best confirmation available — no handset delivery status exists), so it is mapped to `DELIVERED` to mark the schedule as terminal; `EXPD` (validity window expired, ~48h) is terminal and maps to `FAILED` with a refund.

**Two-way SMS (replies / Inbox):** Sends with the "Allow replies" flag (`Schedule.two_way`, per-send checkbox in the UI) are submitted as Welcorp `2 Way SMS` jobs with a reply window (`expires`, default 24h); Welcorp sends them from a pooled virtual number so recipients can reply. Replies are **callback-only** — Welcorp has no endpoint to list or re-fetch them, so a missed webhook is a lost reply, and there is no reconcile/polling fallback for inbound. Reply callbacks POST to the **same** `callback_url` as delivery reports with the same field set, discriminated only by field presence: a reply carries `Response` (the text) and no `Status`. The provider's `parse_inbound_callback()` (base-class hook returning `[]` for providers without two-way support) turns these into `InboundSmsEvent`s; `parse_delivery_callback()` returns `[]` for any Status-less payload so a reply can never be misread as an empty-status delivery failure (which would flip a still-SENT schedule to FAILED + refund — replies usually beat the carrier DLR). The webhook fans replies out to the `process_inbound_message` Celery task, which matches the outbound job via `BroadcastID` → `Schedule.provider_message_id` at any status (batch children disambiguated by the replier's phone; an unmatched job id is dropped with an `inbound_unmatched` warning — an org is never attributed from a phone alone), auto-creates a `Contact` for replies from numbers that have none (raw-number sends are a valid flow), deduplicates provider redeliveries via a content hash (`InboundMessage.dedup_key`, unique), detects opt-out keywords (`INBOUND_OPT_OUT_KEYWORDS`, exact match after strip/upper) and sets `Contact.opt_out=True` org-wide — strictly one-way; no reply can ever opt a contact back in — and stores the reply as an `InboundMessage` (raw webhook payload retained in `raw_data` for forensics). The task never touches schedule status or billing; inbound replies are free. Alphanumeric-sender sends cannot be two-way (the sender ID replaces the reply-capable number) — rejected at the serializer and mutually disabled in the UI. Two-way MMS is not supported (Welcorp's inbound media payload is undocumented). The frontend surfaces replies in an **Inbox** (conversation list + chat thread + reply composer, unread badge on the nav polled every ~20s) and interleaves them into the per-contact message history via the same cursor-paginated thread endpoint.
...
| Conversations | `GET /api/conversations/` (contacts with ≥1 reply, unread counts), `GET /api/conversations/unread-count/`, `GET /api/conversations/:contactId/thread/` (merged outbound+inbound, cursor-paginated via `before`/`next_before`), `POST /api/conversations/:contactId/mark-read/` |
...
| Webhooks | `POST /api/webhooks/clerk/`, `POST /api/webhooks/sms-delivery/` (receives both delivery reports and two-way replies), `POST /api/webhooks/stripe/` |
...
4. Configure your **Webhook** endpoint to point to `https://your-domain/api/webhooks/clerk/` and subscribe to all events below:
   ...
   **Clerk Billing:** `subscription.created`, `subscription.updated`, `subscription.active`, `subscription.pastDue`
...
- When a send registers a callback with the provider, set `callback_registered=True` on the returned `SendResult` (and the top-level key of bulk-send dicts) — `reconcile_stale_sent` uses it to decide how soon to poll (`RECONCILE_NO_CALLBACK_AFTER_MINUTES` vs `RECONCILE_STALE_AFTER_HOURS`); when in doubt leave it `False` (early polling is harmless)
...
### 6. Two-way SMS Limitations

Two-way SMS conversations are implemented (see Architecture → "Two-way SMS"), with known limitations:

- **A missed webhook is a lost reply.** Welcorp delivers replies only via callback — there is no API to list or re-fetch them, so nothing can reconcile a reply lost to backend downtime or a 5xx during the callback POST (Welcorp's retry policy is undocumented). Monitor the `inbound_unmatched` and "neither Status nor Response" log warnings.
- **Local dev cannot receive replies at all.** No `BASE_URL` → no `callback_url` on jobs → Welcorp never calls back, and unlike delivery status there is no polling fallback. Use a tunnel (set `BASE_URL`) or simulate a reply by POSTing the documented form payload to `/api/webhooks/sms-delivery/?token=…` (fields: `BroadcastID`, `Destination` in `61…` format, `Response`, `Timestamp` — see `postInboundReply` in `frontend/e2e/helpers.ts`).
- **Pool-number thread fragmentation on the handset.** Two-way jobs send from Welcorp's shared number pool, so consecutive campaigns may arrive from different numbers and the recipient's phone may split the conversation across threads. Replies to a number whose reply window (`expires`, default 24h) has lapsed may be lost or mis-routed by Welcorp. No code mitigation exists — a dedicated virtual number (a Welcorp account/sales configuration, ~$20/mo to free depending on volume) would fix continuity and is future work.
- **Two-way job pricing is unconfirmed.** Welcorp does not document whether a `2 Way SMS` job costs more than a one-way `sms` job — confirm via the RUNBOOK probe before heavy use; billing currently charges the standard SMS rate.
- **Two-way MMS is unsupported** — Welcorp's inbound media payload is undocumented; MMS sends are always one-way.
- **The Inbox conversation list shows the 50 most recently active conversations** (no pagination controls yet); the unread badge counts org-wide, so a conversation outside the first page can contribute to the badge without being visible. Fine at current scale — add infinite scroll when an org approaches 50 active conversations.
- **Two identical replies in the same second dedupe to one.** Welcorp sends no event id, so redelivery dedup hashes `job|phone|timestamp|text`; the timestamp is second-granularity, so a genuinely repeated identical text within one second (double-tapped "YES") is stored once. Harmless in practice and preferable to storing every webhook redelivery twice.
