...
export function HowItWorksSection() {
  return (
    <section id="how-it-works" className="bg-gray-50 dark:bg-[#0d0030] py-24">
          ...
          {steps.map((item, index) => (
            <div key={item.step} className="relative flex flex-col items-center text-center">
              ...
              {index < steps.length - 1 && (
                <div className="absolute left-1/2 top-8 hidden h-px w-full bg-zinc-200 dark:bg-white/10 md:block" />
              )}
              ...
              <h3 className="mt-6 text-xl font-semibold text-zinc-950 dark:text-white">{item.title}</h3>
              <p className="mt-3 max-w-xs text-sm leading-relaxed text-zinc-500 dark:text-[#a99cc4]">
                {item.description}
              </p>
            </div>
          ))}
