...
// --- @tanstack/react-router mock (shape mirrors send-full.test.tsx) ----------
...
describe('IndexPage — org auto-activation + redirect (B4)', () => {
  ...
  it('renders the loading splash', () => {
    setClerkState()
    render(<IndexPage />)
    expect(screen.getByText('Loading 1Reach application...')).toBeInTheDocument()
  })
  ...
  it('redirects to /app/send on mount', async () => {
    setClerkState()
    render(<IndexPage />)

    await waitFor(() => {
      expect(mockNavigate).toHaveBeenCalledWith({ to: '/app/send' })
    })
  })

  it('still redirects to /app/send even when an org is already active', async () => {
