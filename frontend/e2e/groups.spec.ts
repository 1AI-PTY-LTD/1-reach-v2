...
// Per-spec token so fixtures (contacts, groups) can't collide with other
...
test.beforeAll(async ({ browser }) => {
  ...
  ;[contact1, contact2, contact3] = await Promise.all([
    ensureContact(page, { first_name: `${TOKEN}-Alice`, last_name: 'Smith', phone: '0413111111' }),
    ensureContact(page, { first_name: `${TOKEN}-Bob`,   last_name: 'Jones', phone: '0413222222' }),
    ensureContact(page, { first_name: `${TOKEN}-Removable`, last_name: 'Member', phone: '0413333333' }),
  ])
  group = await createGroup(page, { name: `${TOKEN} VIP Customers` })
...
test.describe('Groups Page', () => {
  test('displays groups list', async ({ page }) => {
    ...
    await expect(page.getByText(`${TOKEN} VIP Customers`).first()).toBeVisible({ timeout: 10000 })
  })
  ...
  test('shows group detail when clicking a group', async ({ page }) => {
    ...
    await expect(page.getByText(`${TOKEN} VIP Customers`).first()).toBeVisible({ timeout: 5000 })
  ...
  test('can open create group modal', async ({ page }) => {
    ...
    // Creating a group is a core feature — assert the button exists rather than
  ...
  test('can fill and submit the create group form', async ({ page }) => {
    ...
    await nameInput.fill(`${TOKEN} Temp E2E Group`)
  ...
  test('can edit group name via edit modal', async ({ page }) => {
    ...
    await nameInput.fill(`${TOKEN} VIP Customers`)
  ...
  test('can remove a member from contacts tab', async ({ page }) => {
    ...
    // Click remove button for the Removable Member contact (aria-label is
    // "Remove <first_name> <last_name> from group", so it carries the token).
    await page.getByRole('button', { name: new RegExp(`Remove ${TOKEN}-Removable Member from group`, 'i') }).click()
