...
class WorkerHealthView(APIView):
    """Liveness of the Celery worker + beat via the dispatch heartbeat.

    dispatch_due_messages writes settings.WORKER_HEARTBEAT_KEY to the broker
    Redis each beat tick — a fresh value proves beat fired AND a worker consumed
    the task. A stale/missing heartbeat means the worker or beat is down (the
    gunicorn-instead-of-celery incident), even when /api/health/ is green.
    The heartbeat is the authoritative signal; an HTTP probe against the api
    container reads it from the shared broker Redis the worker writes to.
    """
    permission_classes = [AllowAny]
    authentication_classes = []
